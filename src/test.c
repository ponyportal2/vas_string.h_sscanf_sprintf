...
START_TEST(memchr_test) {
  ...
  ck_assert_pstr_eq(memchr(str1, test[_i].c, test[_i].n),
                    vas_memchr(str2, test[_i].c, test[_i].n));
}
END_TEST
...
START_TEST(memset_test) {
  ...
  ck_assert_pstr_eq(memset(str1, test[_i].c, test[_i].n),
                    vas_memset(str2, test[_i].c, test[_i].n));
}
END_TEST
...
START_TEST(memcmp_test) {
  ...
  j = vas_memcmp(str4, str2, test[_i].n);
  if (i == -1 || i == 1) {
    if (j < 0)
      j = -1;
    else if (j > 0)
...
START_TEST(memcpy_test) {
  ...
  ck_assert_pstr_eq(memcpy(str1, str3, test[_i].n),
                    vas_memcpy(str2, str4, test[_i].n));
}
END_TEST
...
START_TEST(memmove_test) {
  ...
  ck_assert_pstr_eq(memmove(str1, str3, test[_i].n),
                    vas_memmove(str2, str4, test[_i].n));
}
END_TEST
...
START_TEST(strncat_test) {
  ...
  ck_assert_pstr_eq(strncat(str1, test[_i].str2, test[_i].n),
                    vas_strncat(str2, test[_i].str2, test[_i].n));
}
END_TEST
...
START_TEST(strchr_test) {
  ck_assert_pstr_eq(strchr(test[_i].str2, test[_i].c),
                    vas_strchr(test[_i].str2, test[_i].c));
}
END_TEST
...
START_TEST(strcmp_test) {
  ...
  j = vas_strcmp(test[_i].str1, test[_i].str2);
  if (i == -1 || i == 1) {
    if (j < 0)
      j = -1;
    else if (j > 0)
...
START_TEST(strncmp_test) {
  ...
  j = vas_strncmp(test[_i].str1, test[_i].str2, test[_i].n);
  if (i == -1 || i == 1) {
    if (j < 0)
      j = -1;
    else if (j > 0)
...
START_TEST(strncpy_test) {
  ...
  ck_assert_pstr_eq(strncpy(str1, test[_i].str2, test[_i].n),
                    vas_strncpy(str2, test[_i].str2, test[_i].n));
}
END_TEST
...
START_TEST(strcspn_test) {
  ck_assert_int_eq(strcspn(test[_i].str1, test[_i].str2),
                   vas_strcspn(test[_i].str1, test[_i].str2));
}
END_TEST

// strerror
START_TEST(strerror_test) {
  ck_assert_pstr_eq(strerror(test[_i].n), vas_strerror(test[_i].n));
}
END_TEST

// strlen
START_TEST(strlen_test) {
  ck_assert_int_eq(strlen(test[_i].str1), vas_strlen(test[_i].str1));
}
END_TEST
...
START_TEST(strpbrk_test) {
  ck_assert_pstr_eq(strpbrk(test[_i].str1, test[_i].str2),
                    vas_strpbrk(test[_i].str1, test[_i].str2));
}
END_TEST
...
START_TEST(strrchr_test) {
  ck_assert_pstr_eq(strrchr(test[_i].str1, test[_i].c),
                    vas_strrchr(test[_i].str1, test[_i].c));
}
END_TEST
...
START_TEST(strspn_test) {
  ck_assert_int_eq(strspn(test[_i].str1, test[_i].str2),
                   vas_strspn(test[_i].str1, test[_i].str2));
}
END_TEST
...
START_TEST(strstr_test) {
  ck_assert_pstr_eq(strstr(test[_i].str1, test[_i].str2),
                    vas_strstr(test[_i].str1, test[_i].str2));
}
END_TEST
...
START_TEST(insert_test) {
  ...
  if ((size_t)test[_i].n < strlen(str1)) {
    ...
    strcat(res, res1);
    s = vas_insert(str3, str4, test[_i].n);
...
START_TEST(trim_test) {
  ...
  if (test[_i].str1 != NULL && test[_i].str2 != NULL) {
    char res[SIZE] = "";
    size_t counts = reverser(str1, str2, res);
    char res1[SIZE] = "";
    size_t counte = reverser(res, str2, res1);
    s = vas_trim(str1, str2);
    if (counts + counte != 0)
...
START_TEST(sprintf_test) {
  ...
  vas_sprintf(str2, "Value of Pi = %f", 3.1419);
  ck_assert_pstr_eq(str1, str2);
  sprintf(str1, "%f", 0.2);
  vas_sprintf(str2, "%f", 0.2);
  ck_assert_pstr_eq(str1, str2);
}
END_TEST
...
START_TEST(sscanf_test) {
  ...
  sscanf("123", "%ho", &ho1);
  vas_sscanf("123", "%ho", &ho2);
  ck_assert_int_eq(ho1, ho2);

  sscanf("123", "%lo", &lu1);
  vas_sscanf("123", "%lo", &lu2);
  ...
  sscanf("hello 001 2 3 4 5 10000000000", "%s %hd %2d %d %ld %lx %hx", str1,
         &si, &ni, &i, &li, &lu1, &ho1);
  vas_sscanf("hello 001 2 3 4 5 10000000000", "%s %hd %2d %d %ld %lx %hx", str2,
             &sj, &nj, &j, &lj, &lu2, &ho2);
  ck_assert_pstr_eq(str1, str2);
...
START_TEST(sscanf_extra_test) {
  ...
  vas_sscanf("shrek -0x1 +1 0.05 h 1.0000000001 NAN INF NAN INF 0.003",
...
START_TEST(sscanf_bonus_test) {
  ...
  sscanf(" \t sdsd", "%p", &p);
  vas_sscanf(" \t sdsd", "%p", &p1);

  sscanf(" \t sdsd % 4 010 0.891", "[ \t]%s %% %lo %i %hn %2f", str1, &i, &k,
         &n, &h);
  vas_sscanf(" \t sdsd % 4 010 0.891", "[ \t]%s %% %lo %i %hn %2f", str2, &j,
             &l, &n_, &m);
  ck_assert_pstr_eq(str1, str2);
...
Suite* create_str_suite(size_t size_) {
  Suite* str_suite = suite_create("vas_string_test_suite");
